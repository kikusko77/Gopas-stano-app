...
def scrape_data():
    ...
    file_path1 = filedialog.askopenfilename(filetypes=[("Excel Files", ".xlsx;.xlsb")])
    print(file_path1)
    # Read the first Excel file
    df1 = pd.read_excel(file_path1, engine='pyxlsb')

    # Open a file dialog to select the second Excel file
    file_path2 = filedialog.askopenfilename(filetypes=[("Excel Files", ".xlsx;.xlsb")])

    # Read the second Excel file
    df2 = pd.read_excel(file_path2, engine='pyxlsb')
...
button = tk.Button(root, text="Scrape excel", command=scrape_data, font=("Helvetica", 20), height=2, width=20)
